...
async def on_message(message: discord.Message):
    try:
        ...
        if any(frase in texto.lower() for frase in frases_site):
        ...
        if any(frase in texto.lower() for frase in frases_obras):
        ...
        if any(frase in texto.lower() for frase in frases_capitulos):
        ...
        if should_respond_personal:
            ...
            for chave in saudacoes:
                if texto.lower().startswith(chave) or (mentions_bot and chave in texto.lower()):
                    await message.reply(saudacoes[chave], mention_author=False)
                    return

            if re.search(r"(agradecido|obg|obrigado).*(jeffu)?", texto, re.IGNORECASE):
                await message.reply("Não há de que <:amem:1466774899686117426>", mention_author=False)
                return

            if re.search(r"(te amo|amo vc|amo você).*(jeffu)?", texto, re.IGNORECASE):
                await message.reply("💙 Obrigado... <:shame:1466777359586693376>", mention_author=False)
                return

            # regex robusta para "cala boca" e variações
