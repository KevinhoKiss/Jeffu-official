...
def setup_family_slash_system_v3(bot, owner_id: int, guild_id: int, log_func=None):
    """
    Sistema de famílias via / (slash commands), sincronizado APENAS no servidor informado.
    V3: painel com seleção de família + seleção de membro + botões de criar, ver, renomear,
    mudar cor, foto, ranking, deletar e gerenciar membros sem depender de /familia add/remove.
    """
    if getattr(bot, '_family_slash_v3_registered', False):
        return
    bot._family_slash_v3_registered = True
    ...
    def _build_family_details_embed(guild: discord.Guild, family: dict, selected_member: Optional[discord.Member] = None) -> discord.Embed:
    ...
    async def _add_member_by_panel(interaction: discord.Interaction, selected_slug: str, member: discord.Member):
        data = _load_db()
        guild_data = _guild_bucket(data, interaction.guild_id)
        ...
        if member.bot:
            raise ValueError('Não é permitido adicionar bots em famílias.')
        await _remove_member_from_other_families(interaction.guild, guild_data, member, keep_slug=slug)
        ...
        _save_db(data)
        await _safe_log(interaction.guild, member=interaction.user, title='Membro adicionado à família', channel_name=interaction.channel.name if interaction.channel else '', reason=f'Família: {family.get("name")}', action=f'Membro adicionado: {member}', message_text='')
        return family

    async def _remove_member_by_panel(interaction: discord.Interaction, selected_slug: str, member: discord.Member):
        data = _load_db()
        guild_data = _guild_bucket(data, interaction.guild_id)
        slug, family = _find_family_by_slug(guild_data, selected_slug)
        if not family:
        ...
        _save_db(data)
        await _safe_log(interaction.guild, member=interaction.user, title='Membro removido da família', channel_name=interaction.channel.name if interaction.channel else '', reason=f'Família: {family.get("name")}', action=f'Membro removido: {member}', message_text='')
        return family
    ...
    class MemberUserSelect(discord.ui.UserSelect):
        def __init__(self, parent_view: 'FamilyPanelView'):
            self.parent_view = parent_view
            super().__init__(placeholder='Selecione um membro para add/remove no painel', min_values=1, max_values=1, row=1)
    ...
    class FamilyPanelView(discord.ui.View):
        ...
        @discord.ui.button(label='➕ Membro', style=discord.ButtonStyle.success, row=3)
        async def add_member_button(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            try:
                family = await _add_member_by_panel(interaction, slug, member)
                embed = _build_family_details_embed(interaction.guild, family, member)
                await interaction.response.edit_message(embed=embed, view=self)
                await interaction.followup.send(f'✅ {member.mention} agora faz parte da família **{family.get("name")}**.', ephemeral=True)
            except PermissionError as e:
                await interaction.response.send_message(str(e), ephemeral=True)
            except ValueError as e:
                await interaction.response.send_message(str(e), ephemeral=True)
                ...
                traceback.print_exc()
                await interaction.response.send_message('Falha ao adicionar membro.', ephemeral=True)

        @discord.ui.button(label='➖ Membro', style=discord.ButtonStyle.danger, row=3)
        ...
        async def help_panel(self, interaction: discord.Interaction, button: discord.ui.Button):
            await interaction.response.send_message('Painel V3: selecione a família no primeiro menu e o membro no seletor de usuários. Depois use os botões ➕ Membro ou ➖ Membro para gerenciar os participantes sem depender de `/familia add` e `/familia remove`.', ephemeral=True)

    # ==================== GROUP ====================
    family_group = app_commands.Group(name='familia', description='Sistema de famílias do servidor')

    @family_group.command(name='painel', description='Abre o painel interativo de famílias (V3)')
    async def familia_painel(interaction: discord.Interaction):
        embed = _family_embed(
            'Painel de Famílias • V3',
            'Selecione uma família no menu e um membro no seletor de usuários.\n\nDepois use os botões para visualizar, editar e gerenciar membros diretamente pelo painel.',
        )
    ...
    async def _family_on_ready_sync():
        try:
            ...
            print(f'[FAMILIAS V3] Slash commands sincronizados no servidor {guild_id}: {len(synced)} comando(s)')
        except Exception as e:
            print('[FAMILIAS V3] Falha ao sincronizar slash commands:', e)
            traceback.print_exc()
